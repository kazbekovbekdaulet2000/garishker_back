...
class LessonSerializer(serializers.ModelSerializer):
    ...
    def get_finished(self, obj) -> bool:
        if(self.user.is_anonymous):
            return False
        if(self.user_course):
            progress_lesson = self.user_course.progress_lessons.filter(
                lesson_id=obj.id).first()
            if(progress_lesson):
                return progress_lesson.completed
        return False
    ...
    class Meta:
        ...
        fields = ['id', 'name_kk', 'name_ru', 'description_kk',
                  'description_ru', 'duriation', 'lector', 'order', 'current', 'quiz', 'finished']
...
class LessonDetailSerializer(serializers.ModelSerializer):
    ...
    def get_finished(self, obj) -> bool:
        ...
        if(user.is_anonymous):
            return False
        if(user_course):
            progress_lesson = user_course.progress_lessons.filter(
                lesson_id=obj.id).first()
            if(progress_lesson):
                return progress_lesson.completed
        return False
    ...
    class Meta:
        ...
        fields = ['id', 'name_kk', 'name_ru', 'description_kk', 'description_ru',
                  'duriation', 'organization', 'category', 'lector', 'video', 'order', 'current', 'quiz', 'finished']
